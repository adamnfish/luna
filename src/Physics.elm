module Physics exposing (accelerationDueToGravity, advanceUniverse, distanceBetween)

import Array exposing (Array)
import Model exposing (Acceleration, Body, Position, Universe, Velocity)
...
advanceUniverse prevUniverse =
    let
        bodies =
            arrMapWithOthers advanceBody prevUniverse.bodies
    in
    { prevUniverse
        | bodies = bodies
...
advanceBody body otherBodies =
    let
        ...
        newVelocity =
            applyAcceleration currentAcceleration body.velocity

        newPosition =
            applyVelocity newVelocity body.position
    in
    { body
...
forceDueToGravity body otherBodies =
    let
        ...
        combine otherBody accumulatedAcceleration =
            let
                distance =
                    distanceBetween body.position otherBody.position
